...
import { CheckCircle2, X, File, Image, Video, Music, Archive } from 'lucide-react';
...
interface CompletedTransferModalProps {
  transfer: FileTransfer;
  onSave: () => void;
  onClose: () => void;
}

export function CompletedTransferModal({ transfer, onSave, onClose }: CompletedTransferModalProps) {
  const { t } = useLocale();
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        onClick={onClose}
        className="fixed inset-0 z-45 bg-black/60 backdrop-blur-sm pointer-events-auto"
      />
        ...
        <div className="card w-full max-w-sm pointer-events-auto overflow-hidden">
          ...
          <div className="flex items-center justify-between p-5 border-b border-border">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-sm font-semibold text-primary">{t('completed_title')}</h2>
          ...
          <div className="p-5 flex flex-col items-center gap-4 text-center">
            {transfer.thumbnail ? (
              <div className="w-20 h-20 rounded-xl overflow-hidden border border-white/10 shadow-lg animate-pulse">
